...
## The nine names, then eleven, then fourteen
...
Wave 5 added the tenth and eleventh, `joint_locality_bounded_solution` and
`joint_locality_hyperbolic` (**A19**, at the entry's restated clauses (b′) and (c)); and module B
step 2 added the twelfth, thirteenth and fourteenth on 2026-09-15, `sato_origin_singular`,
`sato_origin_smooth` and `sato_origin_threshold` (**A10**, the three regimes of a
self-decomposable density at the origin, at Sato's letter). Each of the last five has its own
...
/-! ## Chapter 8: the behaviour of a self-decomposable density at the origin (ledger **A10**)

Three names, admitted 2026-09-15 (module B step 2), **the author's decision**, when
`SpatialLine.origin_boundedness` first consumed all three. They are the three regimes of ledger
A10 — singular below the threshold, smooth above it, logarithmic at it — and they are admitted in
the three-name form the P-0008 pass prepared and checked on the same day.

**Why three names and not one.** The entry states three regimes keyed to one constant, and the
consumer reads all three: `cor:origin-boundedness` needs the smooth regime above the threshold,
the singular regime below it and the threshold regime at it. A single conjunction would spend the
same pages and say less about which sentence each proof rests on; three names keep the guard's
lines readable one regime at a time. Wave 6 (2026-09-10) declined an admission for the opposite
reason, that the one assembly then available would have spent one of three and left two names
unspent, which is the campaign's rule against manufacturing a consumer.

**What the three carry beyond their pages**, listed here and at the head of
`blueprint/trust-boundary.txt`, which is the canonical list.

* **The hypothesis is a specification, not a class.** Sato's theorems are stated about a
  self-decomposable law with the given `k`-function; the axioms below quantify over a symmetric
  probability measure pinned by its *cosine transform*, `fourierCos μ ω = exp(−K.exponent ω)`.
  That the two descriptions name the same law is the Lévy–Khintchine correspondence, ledger
  **A3** — whose uniqueness clause is already on this boundary — so the step is a passage through
  a name the article already trusts, not a new analytic fact. It is inside these statements
  because nothing in this development produces "the self-decomposable law of a two-sided profile"
  as an object.
* **The conclusion is a comparison, not the source's asymptotic.** (53.28) and (53.30) give
  `f(x) ∼ [κ sin(k(0+)π)/(Γ(c)sin(cπ))]x^{c−1}K(x)` and `f(x) ∼ (κ/π)cos(c′π/2)L(x)` with their
  constants; what is admitted is the two-sided comparison `c₁g ≤ p ≤ c₂g` on a punctured
  neighbourhood, with the constants and the neighbourhood existential. That is **less** than the
  pages carry — the constants and the cosine factor are not asserted anywhere — and the
  representative is existential, which is the only reading a density admits (review R17).
  **The comparison function `g` is the source's**, and since 2026-09-19 (R171) that is checked
  against the page image rather than paraphrased: `g(x) = |x|^{c−1}K(x)` below the threshold and
  `g(x) = L(x) = ∫_{|x|}^1 K(y)y⁻¹dy` at it, with `K` the slowly varying factor (53.25),
  `K(x) = exp[∫_{|x|}^1 (c − k(y) − k(−y))y⁻¹dy]`. Until that day the singular regime dropped `K`
  and was **false** as typed; the ledger's 2026-09-07 paraphrase of (53.28) had dropped it, and the
  external referee of module B found it.
* **The folding convention is no longer carried.** These statements are in the source's two-sided
  convention, over a `TwoSidedProfile` with the source's `c = k(0+) + k(0−)`; the translation into
  this article's folded profile is `lem:folding-translation`, machine-checked on Lean core as
  `SpatialLine.folding_translation`, and the three theorems that carry the axioms across it are
  `SpatialLine/OriginSingularity.lean`. This is the A21/A22 pattern of row R151 — state the
  interface at the source's letter, prove the translation beside it — and it is what removed the
  second of the two obstacles wave 6 recorded against admitting the entry.

**The hypothesis (53.24) is now on two of the three** (2026-09-19, R171). Sato asks
`c = k(0+) + k(0−) < ∞`, `k(0+) > 0` **and** `k(0−) > 0`; `SatoOriginSingular` and
`SatoOriginThreshold` asked only `0 < c`, which for a two-sided profile is weaker. Both now carry
the two one-sided positivity hypotheses, and the folded consumers supply them, a symmetric law
having `k(0+) = k(0−) = c/2`. `SatoOriginSmooth` is Thm. 28.4 and is left as it was.

**The edges were checked before admission** (wave 6, 2026-09-10, re-read 2026-09-15). `c = 0` is
excluded from all three and must be: with no Gaussian part it forces `k ≡ 0` and the law is `δ₀`,
which has no density. The Gaussian is excluded by the profile having no Gaussian part; `c = ∞` is
excluded by the shape of the hypothesis, a real-valued limit, which matches the entry's own range
and leaves the stable profiles outside; and none of the three is vacuous — the threshold regime is
met by the `Cin` rays, the singular one by `c·1_{(0,τ)}` with `c < 1`, the smooth one by the
Matérn profile at `γ = 1`.

**What is not admitted.** The *rate* at the threshold — that the divergence is logarithmic — is
A16's Bessel asymptotic and is asserted nowhere in this article; the constants of (53.28) and
(53.30); and the propagation of the singularity along the lattice, which is `rem:cin-propagation`,
described and not claimed.

The `Prop`s these axioms inhabit are in `SpatialLine/TwoSidedDefs.lean`: a definitions module, so
that this file can import the vocabulary without importing a chapter 8 theorem module and so
without widening the line paper's release closure past its definitions (the `CornerDefs` /
`VariationDefs` arrangement of R158).
-/

/-- **`prop:cin-origin-singularity`, the singular regime `c < 1`** — cited interface, ledger
**A10** (@sato1999levy Thm. 53.8 with formula (53.28), pp. 410–411).

A self-decomposable law on `ℝ` with no Gaussian part, two-sided profile `k` with `k(0+) > 0` and
`k(0−) > 0`, and `c := k(0+) + k(0−) < 1`, has a density comparable to `|x|^{c−1}K(x)` on a
punctured neighbourhood of the origin, where `K` is the slowly varying factor (53.25),
`TwoSidedProfile.satoK`. The integral in `K` is a genuine one:
`TwoSidedProfile.integrableOn_satoIntegrand`.

**Restated 2026-09-19 (R171); the previous form was false.** It concluded the comparison with
`|x|^{c−1}` alone, the ledger's 2026-09-07 anchor check having paraphrased (53.28) without `K`.
`K` is slowly varying at the origin but need not be bounded: the referee's folded profile
`k(x) = ½ − 1/log(1/x)` on `(0,e^{-4})` has `c = ½`, exponent `½log ω − log log ω + O(1)` and a
kernel of order `|x|^{-1/2}log(1/|x|)`, which no multiple of `|x|^{-1/2}` dominates. The
consumers read only `K ≥ 1` and so keep their conclusions unchanged.

Consumed by `SpatialLine.cin_origin_singularity_unbounded`, and through it by
`SpatialLine.origin_boundedness`. On `blueprint/trust-boundary.txt`. -/
axiom sato_origin_singular : SatoOriginSingular

/-- **`prop:cin-origin-singularity`, the smooth regime `N < c ≤ N+1`** — cited interface, ledger
**A10** (@sato1999levy Thm. 28.4, p. 191).

For an integer `N ≥ 1` and `N < c ≤ N+1` the density has a representative of class `C^{N−1}` on
the whole line. The representative is existential, which is the only reading available for a
density; nothing is asserted about its derivatives at the origin beyond that order.

Consumed by `SpatialLine.cin_origin_singularity_smooth`, and through it by
`SpatialLine.origin_boundedness`. On `blueprint/trust-boundary.txt`. -/
axiom sato_origin_smooth : SatoOriginSmooth

/-- **`prop:cin-origin-singularity`, the threshold `c = 1`** — cited interface, ledger **A10**
(@sato1999levy Thm. 53.8 with formula (53.30), pp. 410–411).

At `c = 1`, with `k(0+) > 0` and `k(0−) > 0`, the density is comparable to the source's
`L(x) = ∫_{|x|}^1 K(y)y^{−1}dy` of (53.26), with `K` the factor (53.25),
`K(y) = exp∫_y^1 (1 − k(u) − k(−u))u^{−1}du` — written here in the two-sided profile, as the
source writes it, and since 2026-09-19 (R171) through the shared `TwoSidedProfile.satoK`, the
singular regime needing the same factor. The order only: neither the constant `κ` of (53.27) nor
the cosine factor `cos(c′π/2)` is asserted, the ledger recording the reading of both as
unresolved. That the comparison function diverges at the origin is *not* part of this axiom: it is
proved from the profile's own monotonicity in `SpatialLine.origin_boundedness`. Nor is the *rate*
of the divergence: `L` is a constant multiple of `log(1/|x|)` only where `K` is constant near the
origin, and with `K ≍ log(1/x)` it is of order `log²`.

Consumed by `SpatialLine.cin_origin_singularity_threshold`, and through it by
`SpatialLine.origin_boundedness`. On `blueprint/trust-boundary.txt`. -/
axiom sato_origin_threshold : SatoOriginThreshold

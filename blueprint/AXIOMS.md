...
**The sources' own wording** is in `AXIOMS-verbatim.md` beside this file: the librarian's audit of 2026-09-19, every anchor of every entry transcribed verbatim from the held copy (page images), set against the entry's "statement as used" and the Lean axiom, with a verdict. It was made after entry A10 turned out to have been verified from a paraphrase (row R171). Result: no entry claims more than its pages; A20's uniqueness clause differs from Huber's printed Theorem 2 (no boundedness hypothesis is displayed there), as the entry already disclosed, and the reread of Huber's §§ 1–2 is owed before module C relies on `rem:gaspt-uniqueness`. A new entry, or a changed "statement as used", gets its verbatim block there in the same commit.
...
**Blueprint:** `prop:fourier-toolbox` (clause 2); `prop:dimension-filtration` (clause 3, read at the anchor in dimension `d`, 2026-09-17) · **Lean:** *(not stated yet)*
...
**Blueprint:** `prop:fourier-toolbox` (clause 3); `prop:dimension-filtration` (clause 2, read at the anchor in dimension `d`, 2026-09-17: Thm. 8.1 is stated on `ℝ^d`); `lem:filtration-strictness` (clause 3, the same reading in dimension `d`, 2026-09-19, R191 — prose-only, no Lean) · **Lean:** `SpatialLine.fourier_toolbox_levy_converse`, `SpatialLine.fourier_toolbox_levy_unique`, both in `Formalization/SpatialLine/Interfaces.lean` *(the converse and uniqueness clauses; existence is not stated as an axiom — nothing consumes it yet)*
...
**Blueprint:** `prop:sd-exponents`; `prop:dimension-filtration` (clauses 1–3, 2026-09-17: Def. 15.1, Prop. 15.5 and the polar criterion Thm. 15.10 read on `ℝ^d`, where the source states them); `lem:filtration-strictness` (clause 3, the polar criterion at both of its directions in dimension `d`, 2026-09-19, R191 — prose-only, no Lean) · **Lean:** *(not stated yet)*
...
- **2026-09-19, read verbatim for `prop:dimension-filtration`(4)** (librarian, page images; R172). *Def. 15.1, p. 90:* `μ` on `ℝ^d` is selfdecomposable if, for any `b > 1`, there is a probability measure `ρ_b` on `ℝ^d` with `μ̂(z) = μ̂(b⁻¹z) ρ̂_b(z)` (15.1). *Prop. 15.5, p. 93:* "If μ is selfdecomposable, then μ is infinitely divisible and, for any b > 1, ρ_b in (15.1) is uniquely determined and infinitely divisible." The clause about `ρ_b` is what clause (4) spends, in every dimension. *Cor. 15.11, p. 95,* verbatim: `μ` on `ℝ` is selfdecomposable iff `μ̂(z) = exp[−½Az² + iγz + ∫(e^{izx} − 1 − izx 1_{[−1,1]}(x)) k(x)/|x| dx]` with `A ≥ 0`, `γ ∈ ℝ`, `k ≥ 0`, `∫(|x|²∧1) k(x)/|x| dx < ∞`, `k` increasing on `(−∞,0)` and decreasing on `(0,∞)`. For the half-line step the node also reads *Thm. 24.11, p. 153* (a Lévy process is a subordinator iff `A = 0`, `S_ν ⊂ [0,∞)`, `∫_(0,1] x ν(dx) < ∞`, `γ₀ ≥ 0`, with the Laplace transform (24.5)) and *Remark 21.6, p. 138* (`E[e^{−uX_t}] = exp[t(∫_(0,∞)(e^{−ux} − 1)ν(dx) − γ₀u)]`, which is `exp[−t(γ₀u + ∫(1 − e^{−ux})ν(dx))]`).
...
**Blueprint:** `prop:cin-origin-singularity`, `cor:origin-boundedness`, `cor:origin-smooth` *(the last since 2026-09-19, R189: the regime `c = ∞`, **prose-only** — no Lean is scheduled for it and no name is admitted for it, the three Lean names below being the regimes at finite `c`)* · **Lean:** `SpatialLine.sato_origin_singular`, `SpatialLine.sato_origin_smooth`, `SpatialLine.sato_origin_threshold` *(in `Formalization/SpatialLine/Interfaces.lean`; **admitted 2026-09-15, the author's decision**, one name per regime, each stated **at Sato's letter** over the source's own two-sided profile. The translation into this article's folded convention is `lem:folding-translation`, machine-checked on Lean core, and is **not** carried by the axioms. The four declarations that spend them — `SpatialLine.cin_origin_singularity_unbounded`, `_smooth`, `_threshold` and `SpatialLine.origin_boundedness`, in `SpatialLine/OriginSingularity.lean` — are all proved, and the corresponding `Skeleton` declarations are gone)*
**Cite:** @sato1999levy — Thm. 28.4, p. 191 (the smooth regime, `N < c ≤ N+1`, at its clause (i), and the regime `c = ∞` at its clause (ii)); Thm. 53.8, pp. 410–411, hypothesis (53.24) and formulas (53.25)–(53.28) and (53.30) (the singular regime and the threshold `c = 1`; (53.25) is the slowly varying factor `K` both comparison functions carry, and (53.26) is `L`)
...
- **Statement as used.** For a self-decomposable law with no Gaussian part and two-sided
  `k`-function, with `c := k(0+) + k(0−)` and both one-sided limits positive: for `c < 1` the
  density is unbounded with `f(x) ≍ |x|^{c−1}K(x)`, `K` the slowly varying factor (53.25)
  displayed in the bullet of 2026-09-19 below; for `N < c ≤ N+1` it extends continuously with
  `N − 1` derivatives; at `c = 1` it is unbounded with `f(x) ≍ L(x)`, `L` the integral (53.26) of
  the same `K`; **and for `c = ∞` the law has a `C^∞` density on `ℝ`** (Thm. 28.4(ii);
  the clause added 2026-09-19, R189, for `cor:origin-smooth`). *(Corrected 2026-09-19, R171: this
  bullet said `f(x) ≍ |x|^{c−1}` below the threshold, dropping `K`, and said "continuous at no
  better than logarithmic order" at the threshold, where `L` is of logarithmic order only when
  `K` is constant near the origin.)*
...
- **Three matchings of the source's hypotheses, and one of its conclusion (2026-09-19, R180,
  R181; the second external review round, referee A findings 4–6).** Sato's Thm. 53.8 is stated
  for a law of the driftless finite-variation form (53.14) with (53.15)
  `∫_{|x|<1}k(x)dx < ∞`, and his `k`-function is by definition the version left-continuous on
  `(0,∞)` and right-continuous on `(−∞,0)` (p. 403, repeated at Thm. 28.4, p. 191). Neither is a
  hypothesis of the three admitted specifications, and both hold for the data this article feeds
  them: (53.15) because `k ≤ c < ∞` under the finiteness hypothesis each specification carries,
  the drift because the law is specified through its cosine transform and the Gaussian part
  because `a = 0`; the continuity normalisation because passing to the one-sided-continuous
  modification changes `k₂` on a countable set only, hence changes neither the Lévy measure, nor
  the exponent, nor the two one-sided limits, nor `K` and `L`. The modification step is what the
  three names carry beyond their pages on this count and is listed at the head of
  `blueprint/trust-boundary.txt`. On the conclusion side, the specifications' existential
  two-sided comparison is *weaker* than the source's asymptotic **only because the source's
  constant does not vanish**: `f ~ Ag` gives `f ≍ g` only for `A ≠ 0`, and here
  `A = κ sin(k₂(0+)π)/(Γ(c) sin(cπ))` with `k₂(0+) = c/2` gives
  `sin(cπ/2)/sin(cπ) = 1/(2cos(cπ/2)) > 0` below the threshold, and `cos(c'π/2) = 1` at it. Both
  are consequences of the symmetry. `lem:folding-translation`'s annotation and
  `prop:cin-origin-singularity`'s carry these in print.
...
- **2026-09-15, module B step 2, R159: the entry has a consumer, the translation is checked, and
  the admission is prepared but not taken** (the postdoc's P-0008). Three things in this
  repository said the same thing on the line and had never been put side by side: the external
  review's finding C, that the closed form of `prop:matern-density` is a Matérn *covariance* only
  for `γ > 1/2` and is `K_0` at `γ = 1/2`; this entry's three regimes, whose boundary is `c = 1`;
  and the Matérn profile's own value at the origin. They are one statement, and it is a statement
  about the *profile*: **an admissible kernel with no Gaussian part is bounded at the origin
  exactly when its folded `k(0+)` exceeds `1`.**
  - **The consumer** is `cor:origin-boundedness` (`blueprint/src/parts/08-cone.tex`), typed as
    `Skeleton.origin_boundedness` and `notready`. It reads **all three** regimes — the smooth one
    above the threshold, the singular one below it, the threshold one at it — which is exactly
    what the wave-6 note found missing when it observed that the one assembly then available
    would spend one of three and leave two unspent.
  - **The witness** is the Matérn member, and it is machine-checked on Lean core:
    `SpatialLine.matern_origin_constant` (`SpatialLine/MaternThreshold.lean`) gives
    `k(0+) = 2γ` for the datum of `prop:matern-exponent`(1), together with both one-sided limits
    of its two-sided profile and the three iffs `2γ < 1 ↔ γ < 1/2`, `2γ = 1 ↔ γ = 1/2`,
    `1 < 2γ ↔ 1/2 < γ`. So this entry's threshold lands at `γ = 1/2`, which is where finding C
    independently put it from the closed form. The agreement of the two is the check that the
    folding translation below is the right one; A8's own bullet above already recorded the same
    coincidence from a third side, the Matérn members at `γ ≤ 1/2` being the counterexample that
    narrowed `kernel_regularity_law` to the open half-line.
  - **The folding translation is no longer carried by the entry.** It is
    `lem:folding-translation`, a `[T]` node of chapter 8, machine-checked on Lean core as
    `SpatialLine.folding_translation` (`SpatialLine/TwoSidedProfile.lean`): for a two-sided
    profile `k₂(x) = k(|x|)/2`, the two-sided exponent is the folded one, `k₂(x) + k₂(−x) = k(x)`
    for `x > 0`, and both one-sided limits at the origin are `c/2`, so the source's
    `c = k₂(0+) + k₂(0−)` is the folded `k(0+)` and the source's `c′` vanishes. The "what the
    entry does NOT carry" bullet above stands as a statement about the *citation*; what changed
    is that the step it names is now proved here rather than asserted.
  - **The admission is prepared and not taken.** `SatoOriginSingular`, `SatoOriginSmooth` and
    `SatoOriginThreshold` (`SpatialLine/TwoSidedProfile.lean`) are the three regimes typed **at
    Sato's letter** — quantified over a `TwoSidedProfile`, with `c` the sum of the two one-sided
    limits and (53.30)'s comparison function written in `k₂(u) + k₂(−u)`. They are `Prop`-valued
    *definitions*, not axioms: `blueprint/trust-boundary.txt` is untouched and no name was added
    to the trust boundary. That the admission would suffice is itself machine-checked:
    `SpatialLine.cin_origin_singularity_unbounded_of_sato`, `_smooth_of_sato` and
    `_threshold_of_sato` derive the node's three folded statements from the three specifications
    *taken as hypotheses*, on Lean core, through the translation. **Admitting a name is the
    author's decision**; the exact `axiom` block and the per-name step it would carry are in the
    session report and in `Formalization/SKELETON.md` § 25.
  - **Not settled here**: the *rate* at the threshold (that the divergence is logarithmic) is
    A16's Bessel asymptotic and is asserted nowhere; the constant of (53.30), which the
    narrowing below still excludes; and the `d`-dimensional half of P-0008, which is Paper VII's.
- **2026-09-15, the author's decision: the entry is admitted, in three names** (module B step 2,
  ledger row R162; `Formalization/SKELETON.md` § 26). The three names are the three regimes, one
  each, and they are the twelfth, thirteenth and fourteenth on `blueprint/trust-boundary.txt` —
  the first admitted since the v0.1 release and the first from chapter 8.
  - **What each name carries beyond its pages** is one list of three items, the same for all
    three, and it is at the head of `trust-boundary.txt`, which is canonical. In short: *(a)* the
    hypothesis is this article's cosine-transform specification of a symmetric law rather than
    the source's self-decomposable law — a passage through **A3**, already on the boundary, and
    inside the statement because nothing here produces "the self-decomposable law of a two-sided
    profile" as an object; *(b)* the conclusion is the two-sided comparison on a punctured
    neighbourhood with an existential representative, which is **less** than (53.28) and (53.30)
    carry, neither constant nor the cosine factor being asserted; *(c)* the folding convention is
    **no longer carried** — the three are stated over a `TwoSidedProfile` with the source's
    `c = k(0+) + k(0−)`, and `lem:folding-translation` carries them into this article's folded
    convention on Lean core.
  - **Why now.** The two reasons wave 6 recorded for not admitting are the two things that
    changed: `cor:origin-boundedness` spends **all three** regimes, so no name is admitted
    unspent; and the folding translation is a checked theorem beside the interface rather than a
    step inside it. Stating an interface at its source's letter and proving the translation
    beside it is the A21/A22 pattern of row R151, here applied at the admission rather than
    after it.
  - **What did not change.** Both headline `#print axioms` blocks are byte-identical across the
    admission — neither `main_characterization` nor `scale_monotone_noncreation` reaches chapter
    8's origin analysis — and no other node's status moved. `prop:cin-origin-singularity` and
    `cor:origin-boundedness` are `\leanok`.
- **2026-09-19, R171: the singular regime was FALSE as typed, and the axiom with it; the entry is
  restated at the source's letter** (the external review of module B; `SKELETON.md` § 32). The
  referee found that the singular comparison `f(x) ≍ |x|^{c−1}` drops a slowly varying factor.
  It does, and the factor is on the page: **the librarian re-read Thm. 53.8 from the page image on
  2026-09-19**, and this is the theorem verbatim.
  > Let `μ` be a selfdecomposable distribution on `ℝ` satisfying (53.14) with `k`-function `k(x)`
  > satisfying (53.24) `c = k(0+) + k(0−) < ∞, k(0+) > 0, and k(0−) > 0`. Let
  > `c' = k(0+) − k(0−)`. Define (53.25) `K(x) = exp[∫_{|x|}^1 (c − k(y) − k(−y)) dy/y]`, `x ≠ 0`;
  > (53.26) `L(x) = ∫_{|x|}^1 K(y) dy/y`, `x ≠ 0`; (53.27)
  > `κ = exp[c∫_0^1(e^{−x}−1)dx/x + c∫_1^∞ e^{−x}dx/x − ∫_1^∞(k(x)+k(−x))dx/x]`. Then the density
  > `f(x)` of `μ` satisfies: (i) If `c < 1`, then (53.28)
  > `f(x) ~ [κ sin(k(0+)π)] / [Γ(c) sin(cπ)] · x^{c−1} K(x)` as `x ↓ 0` and (53.29)
  > `f(x) ~ [κ sin(k(0−)π)] / [Γ(c) sin(cπ)] · |x|^{c−1} K(x)` as `x ↑ 0`. (ii) If `c = 1`, then
  > (53.30) `f(x) ~ (κ/π) cos(c'π/2) L(x)` as `x → 0` and (53.31)
  > `lim_{x↓0} [f(x) − f(−x)]/K(x) = κ sin(c'π/2)`.

  In the proof of Thm. 53.6 (p. 410) the one-sided `K` of (53.18) is said to be slowly varying at
  `0`. So **(53.28) carries `K(x)`**, and this entry's paraphrase — written at the anchor check of
  2026-09-07 and repeated in the node and in the axiom's docstring — dropped it, while the
  "deliberate narrowing" note discussed only the constant and the cosine factor. **The axiom
  `SpatialLine.sato_origin_singular`, admitted on 2026-09-15, was therefore false from its
  admission until 2026-09-19.** The referee's witness: the folded profile
  `k(x) = ½ − 1/log(1/x)` on `(0,e^{-4})` and `0` beyond is admissible with `c = ½`; its exponent
  is `F(ω) = ½log ω − log log ω + O(1)`, so `e^{−F} ≍ ω^{−1/2}log ω`, and convolving a density
  bounded by `C|x|^{−1/2}` with a Gaussian of width `ε` gives `O(ε^{−1/2})` where the positive
  Fourier integral is at least a multiple of `ε^{−1/2}log(1/ε)`.
  - **What changed.** `SatoOriginSingular`'s conclusion is `c₁|x|^{c−1}K(x) ≤ p ≤ c₂|x|^{c−1}K(x)`
    with `K` the shared definition `TwoSidedProfile.satoK` of (53.25); `SatoOriginThreshold` reads
    its `L` through the same definition; and **both** now carry (53.24)'s hypotheses `k(0+) > 0`
    and `k(0−) > 0`, which the folded consumers supply, a symmetric law having
    `k₂(0+) = k₂(0−) = c/2`. `SatoOriginSmooth` (Thm. 28.4) is untouched. The restatement of an
    interface at its source's letter is delegated (the author's decision of 2026-09-11), and it is
    reported as a decision taken.
  - **No consumer weakened.** `cor:origin-boundedness` reads one property of `K`, `K ≥ 1`, which
    is machine-checked on Lean core (`SpatialLine.one_le_satoK`) from `k ≤ c` on `(0,∞)`; so
    `prop:cin-origin-singularity` and `cor:origin-boundedness` keep their conclusions and their
    `\leanok`. What the printed statement gains is the factor, and what the annotation gains is
    the warning that `K` is slowly varying and **not** bounded in general, with the referee's
    example.
  - **A junk-value exposure, named and closed.** `K` is an exponential of a Bochner integral, and
    Bochner's integral of a non-integrable integrand is `0`, which would make `K ≡ 1` and return
    the axiom to its false form in the degenerate case. The integrand is integrable on `(y,1)` for
    every `y > 0` and every `c`, the profile being monotone on each piece, and that is now proved
    beside the definition (`TwoSidedProfile.integrableOn_satoIntegrand`, Lean core). The same point
    was load-bearing for the threshold regime at the admission (`SKELETON.md` § 26), where `L(x)`'s
    integrand needed the antitonicity of `K`.
  - **Not settled here**: the rate at the threshold, which is logarithmic only where `K` is
    constant near the origin — with `K ≍ log(1/x)` one gets `L ≍ log²(1/x)` — so the blueprint's
    "logarithmic" now names the `Cin` rays and the Matérn member at `γ = ½` and not the regime; and
    the constants of (53.28) and (53.30), which are read above and are still not asserted anywhere.
- **DELIBERATE NARROWING.** The blueprint node claims the *order* of the singularity and not the
  ...
  pass must settle both before the constant may be asserted anywhere. **This survives the
  admission and is now enforced by the admitted types**: `SatoOriginSingular` and
  `SatoOriginThreshold` (`SpatialLine/TwoSidedDefs.lean`) conclude `c₁g ≤ p ≤ c₂g` with `c₁`,
  `c₂` and the neighbourhood existential, so neither constant nor the cosine factor is asserted
  anywhere in the Lean either.
- **2026-09-19, R189: the entry is extended to `c = ∞`, and the extension is admitted nowhere**
  (the author's decision D1 at the second external review round of module B). Until today the
  article claimed nothing about the kernel at an unbounded profile, which is the case of the
  stable members and of every completely monotone profile with an unbounded Thorin measure; the
  four headline sentences about the boundedness threshold had been narrowed in batch S2 for
  exactly that reason. Thm. 28.4's clause (ii) settles it in one sentence, and it was read from
  the page image by the librarian on 2026-09-19 (`notes/reviews/cone/SOURCES-round2-2026-09-19.md`,
  Part 1.1) and transcribed verbatim into `blueprint/AXIOMS-verbatim.md` in the same change:
  > (ii) If `c = ∞`, then `μ` has a `C^∞` density on `ℝ`.

  The theorem's own hypotheses, as printed, are: `μ` selfdecomposable on `ℝ` of the form (28.3)
  — the general purely non-Gaussian self-decomposable form, Cor. 15.11 — with `k` nonnegative,
  right-continuous and increasing on `(−∞,0)`, left-continuous and decreasing on `(0,∞)`, and
  `c = k(0+) + k(0−) > 0`. **No finite-variation condition and no condition on the drift**: those
  belong to Thm. 53.8's (53.14)–(53.15) and are what the *finite*-`c` regimes need, which is why
  they are not asked for here. The consumer is `cor:origin-smooth` (chapter 8), `[A]`,
  `notready`, prose-only: the clause is **not** admitted as a Lean axiom, no name is added to
  `blueprint/trust-boundary.txt`, and the three admitted names remain the regimes at finite `c`.
  What the node carries beyond the page is the folding translation (two lines, written out in its
  proof — `lem:folding-translation` itself is not available, its hypothesis being `c < ∞`) and the
  same one-sided-continuity modification the three admitted names carry.
...
**Blueprint:** `prop:moments-tails` (clause 1); `prop:rational-closure` (clause 5, at the submultiplicative weight `e^{β|x|}` for the exponential moment, 2026-09-19, R200 — prose-only, no Lean) · **Lean:** `SpatialLine.moments_tails_criterion` (`SpatialLine/Interfaces.lean`, on `blueprint/trust-boundary.txt`), consumed by `SpatialLine.stable_family_moments` and `SpatialLine.matern_moments_integrable`
...
**Blueprint:** `lem:thorin-ggc` *(the step it does not carry, the pairing of `±θ` at `σ = iω`, is the separate node `lem:thorin-two-sided`, Lean core since 2026-09-15)* · **Lean:** *(not admitted — no consumer, and none is scheduled; see the wave-6 note and the R160 note below)*
...
- **What the entry does NOT carry.** The pairing of `±θ` at `σ = iω`, which gives
  ...
  integrability conditions; and clauses (3) and (4) of `prop:thorin-subclass` entirely (the
  bridge and the strictness; the clause numbering of that node was 1, 2, 4, 5 until 2026-09-19,
  R175). All **[T]**.
- **The comparison of the conditions is an equivalence and not term by term (2026-09-19, R175).**
  `lem:thorin-ggc`'s proof said "term by term", which is false: the source's (7.1.2) asks
  `∫(1+t²)^{-1}U(dt) < ∞` over all of `ℝ∖{0}` — which bounds `U` on every compact set — together
  with `∫_{|t|≤1}|log t²|U(dt) < ∞`, while `eq:thorin`'s condition ran over `(0,1]` and
  `(1,∞)` separately and controlled nothing near `θ = 1`. `eq:thorin` now prints the single
  integral `∫log(1 + θ^{-2})U(dθ) < ∞`, which is equivalent to the source's pair under the fold,
  each bounding the other by an elementary comparison of integrands (written out in the proof).
...
- **The pairing is machine-checked, and what is left here is a name (2026-09-15, module B
  step 2, R160).** P-0010 asked whether module B's Thorin section can be written without
  admitting this entry. It can, and the answer did not need a new interface: what the entry
  itself says it does not carry — the pairing of `±θ` at `σ = iω` — is now the node
  `lem:thorin-two-sided`, `\leanok` on **Lean core** (`SpatialLine/ThorinTwoSided.lean`:
  `thorin_pair_mul`, `neg_log_norm_thorin_factor`, `thorin_pair_compensator`,
  `thorin_two_sided_fold`). `lem:thorin-ggc` is **narrowed** to what this entry does carry, the
  reading of Bondesson's definition at imaginary argument, and stays `notready` with no Lean and
  no consumer. So the entry grounds a naming convention and no step of any proof. It stays
  unadmitted, and the admission question is closed rather than deferred.
- **Three findings from writing the pairing down.** (i) The product of the two factors at
  `±θ` is the *positive real* `(1 + ω²/θ²)⁻¹`, so no branch of the complex logarithm has to be
  chosen; the printed proof's sum of two complex logarithms was avoidable. (ii) The modulus of a
  *single* factor already gives `½log(1 + ω²/θ²)` at every `θ ≠ 0` — the folding convention's
  factor `2` comes from there, and the symmetry of `V` is needed only to cancel the imaginary
  parts and the compensators. (iii) The fold is cleanest stated at the measure,
  `U = 2V|_{(0,∞)}`, so that the `½` of `(10.1)` absorbs it.

## A18 — The generalized inverse Gaussian laws are generalized gamma convolutions
**Blueprint:** `rem:student-ggc` *(2026-09-15, R160: the entry's two statement nodes, `prop:student-t`(3) and `lem:student-subordinated`, quantify over the representation instead, so the entry now grounds this cited remark alone; before that it was `prop:student-t`, and `lem:student-subordinated` from the R121 split of 2026-09-11, itself replacing `lem:subordinated-members` from the R44 split the same day)* · **Lean:** *(not admitted — no consumer, and none is wanted; see the R160 note below)*
...
- **Both consumers are restated as conditionals and proved; the entry stays unadmitted and now
  grounds a cited remark alone (2026-09-15, module B step 2, R160).** P-0010 asked whether the
  Student-t corner can be carried at the level of Thorin measures. It can, and both nodes are
  `\leanok` in what they now assert. `lem:student-subordinated` quantifies over a causally
  admissible datum whose delay law is the inverse-gamma law of shape `a` and concludes membership
  in `S` (`SpatialLine.student_subordinated`, **Lean core**). `prop:student-t`(3) quantifies over
  the *Thorin measure* at this entry's own letter — Halgreen's (2) and (3), p. 13, at
  `λ = −a < 0`, `χ = 1`, `ψ = 0` — and concludes self-decomposability, complete monotonicity of
  the spatial profile and the spatial Thorin representation at twice the image of `U_I` under
  `θ ↦ √(2θ)` (`SpatialLine.student_thorin`, which spends A3 and A11 through `thorin_bridge` and
  no name of its own). The existence of the representation is the new remark `rem:student-ggc`,
  marked not machine-checked, and that is where this entry now binds: no statement node cites it,
  no declaration reads it, and `Skeleton.student_causal` and `Skeleton.student_subordinated` are
  withdrawn.
- **The interface as typed could not have discharged the clause it was recorded against
  (2026-09-15, R160; found while writing the statement).** `Skeleton.student_causal` asserted the
  existence of a `CausalAdmissible` with the inverse-gamma Laplace transform — self-decomposability
  — while `prop:student-t`(3) concludes that the *spatial* profile is completely monotone, which
  needs the *causal* profile to be a Laplace transform, i.e. the generalized-gamma-convolution
  half of this entry. The Lean shape of the interface dropped exactly what its consumer consumes,
  so admitting it would have left the clause unprovable; the hypothesis now carries the Thorin
  measure. What `lem:student-subordinated` needs is the weaker fact, and it is stated with the
  weaker hypothesis.
- **What a Lean admission would have to say, if one is ever wanted.** At the source's letter:
  `axiom inverseGamma_ggc (a : ℝ) (ha : 0 < a) : ∃ U : Measure ℝ, IsFolded U ∧
  causalThorinExponentL U 1 ≠ ⊤ ∧ ∀ σ : ℝ, 0 ≤ σ → ∫ u, Real.exp (-(σ * u)) ∂(inverseGammaLaw a)
  = Real.exp (-causalThorinExponent U σ)` — Halgreen §2, (2), (3) and (5), pp. 13–15. It would
  carry, beyond those pages, the identification of `p_{T_1}` with his (1) at those parameters and
  the compact form of his (3) (finiteness of the exponent at `σ = 1`, which is his two conditions
  for a measure finite on the compact subsets of `(0,∞)`). It is **not** proposed: with both
  consumers conditional, it has no Lean consumer either.
...
**Blueprint:** `prop:dimension-filtration` (clause 3) · **Lean:** *(not stated; no Lean is scheduled for the node, the article having no development of laws on `ℝ^d`)*
**Cite:** @schilling2012bernstein — Thm. 13.14, p. 212 (the radial theorem, "Schoenberg; Bochner"); Def. 3.1 and Thm. 3.2, p. 21 (the definition of a Bernstein function and its representation with a unique triplet)

- **Anchor verified 2026-09-17** (librarian, read from the held copy). Thm. 13.14, p. 212, as printed: *a function `f : (0,∞) → [0,∞)` is a Bernstein function if, and only if, for all `d ∈ ℕ` the function `ξ ↦ f(|ξ|²)`, `ξ ∈ ℝ^d`, is continuous and negative definite.* The domain is `(0,∞)`, the quantifier is "for all `d`", and no continuity of `f` itself is asked; it is the necessity direction (Bernstein from negative definite in every dimension) that takes a limit in the dimension, through vague compactness on p. 213, the sufficiency direction being Example 13.13. Def. 3.1 and Thm. 3.2 are both on p. 21: a Bernstein function is `C^∞` on `(0,∞)`, nonnegative, with `(−1)^{n−1} f^{(n)} ≥ 0` for all `n ≥ 1`; and `f` is a Bernstein function if and only if `f(λ) = a + bλ + ∫_{(0,∞)} (1 − e^{−λt}) μ(dt)` with `a, b ≥ 0` and `∫ (1 ∧ t) μ(dt) < ∞`, the triplet `(a, b, μ)` determining `f` uniquely and vice versa.
- **Statement as used.** For an admissible `F` whose radial extension `ξ ↦ F(|ξ|)` is a Lévy exponent on `ℝ^d` for every `d`, the function `σ ↦ F(√(2σ))` is a Bernstein function on `(0,∞)`, hence has the displayed representation; the node then differentiates the representation (a dominated-convergence step of its own) to conclude `F' > 0` off the origin. Only this direction of the radial theorem is spent.
- **What the entry is for.** The top of the dimension filtration, `𝒜_∞ ⊆ ℬ`, which places the lattice-step members of the line's theory (the `Cin` rays among them) outside every radial restriction valid in all dimensions and outside the type G laws. It is the one fact of module B's bridge section that is cited rather than proved, beside A11–A12 at `rem:bridge-subordination`, which no statement reads.
- **What the entry does NOT carry.** The hypothesis of the theorem is supplied by the node from A2 and A7 read in dimension `d` (a self-decomposable law is infinitely divisible, and the exponent of an infinitely divisible law is continuous and negative definite in the kernel sense); the identification of `ℬ` with the type G laws in the sense of variance mixtures (@steutel2004infinite, (2.7), p. 345), which is `rem:dimension-filtration`'s reading; and the converse direction of the theorem, which nothing spends.
- **Not admitted (2026-09-17).** The entry grounds a prose-only `[A]` node; no Lean name is declared and none is on the trust boundary. The author's decision at the review of module B's section 4 (the remark's proof-shaped paragraphs to become a proposition with a proof) is what opened the entry; fidelity row R166.

## A24 — the closure of the generalized gamma convolutions, with the continuity theorem for Laplace transforms
**Blueprint:** `prop:rational-closure` (clauses 1 and 4; clause 4 added 2026-09-19, R200, and spends the same two facts) · **Lean:** *(not stated; chapter 14 has no Lean scheduled)*
**Cite:** @bondesson1992generalized — Thm. 3.1.5 (Closure theorem), pp. 34–35, with its reformulation (i)–(iii) on p. 35; the definition of the class, (3.1.1)–(3.1.2), p. 29 · @feller2009introduction — Vol. 2, XIII.1, Thm. 2 (Continuity theorem), p. 431

- **Anchors verified 2026-09-19** (librarian, read from the held copies, from the page images where the text layer is unreliable). *Bondesson, p. 29:* a generalized gamma convolution is a probability distribution `F` on `[0,∞)` with moment generating function `φ(s) = exp{as + ∫ log(t/(t−s)) U(dt)}`, `s ≤ 0`, where `a ≥ 0` and `U` is a nonnegative measure on `(0,∞)` with `∫_(0,1] |log t| U(dt) < ∞` and `∫_(1,∞) t⁻¹ U(dt) < ∞`. *Thm. 3.1.5, pp. 34–35*, in the book's parametrization by a measure `ν` on `[0,∞]` with `ν({0}) = 0`, `ν({∞}) = a`, `ν(dt) = log(1+t⁻¹) U(dt)`: if `F_n ∈ 𝒯` tend weakly to a non-defective `F`, then `F ∈ 𝒯` and its measure `ν` is a vague limit of the `ν_n` on `[0,∞]`; conversely vague convergence `ν_n → ν` with `ν({0}) = 0` gives weak convergence. *Reformulated on p. 35, as printed:* `F_n → F ∈ 𝒯` weakly iff (i) `U_n → U` vaguely on `(0,∞)`, (ii) `a = lim_{A→∞} lim_{n→∞} (a_n + ∫_(A,∞) t⁻¹ U_n(dt))`, (iii) `lim_{ε→0} lim_{n→∞} ∫_(0,ε) (log t⁻¹) U_n(dt) = 0`; "if (iii) fails to hold then the limit distribution is defective". The iterated limits are the book's. *Feller, p. 431, Thm. 2 as printed:* for probability distributions `F_n` with transforms `φ_n`, if `F_n → F` with `F` possibly defective then `φ_n(λ) → φ(λ)` for `λ > 0`; conversely, if `φ_n(λ)` converges for each `λ > 0` to a limit `φ(λ)`, then `φ` is the transform of a possibly defective distribution `F` and `F_n → F`; the limit is not defective iff `φ(λ) → 1` as `λ → 0`.
- **Statement as used.** Finite convolutions of gamma laws with integer shapes, whose Laplace transforms converge pointwise on `[0,∞)` to `e^{−G}` with `G` continuous and `G(0) = 0`, converge weakly to a probability law with that transform (Feller), which is a generalized gamma convolution whose Thorin measure is the vague limit on `(0,∞)` of theirs (Bondesson, the closure half and clause (i)). Clause (ii), that mass escaping to infinity becomes the left extremity, is what makes a Gaussian part appear in the limit on the line; the node does not need it beyond `a ≥ 0`.
- **What the entry does NOT carry.** That a vague limit of integer-valued point measures is one, which the node proves; the passage between the line and the half-line, which is `prop:thorin-subclass`(4) read at `σ = ω²/2`; the converse clause of the node, which is `prop:levy-continuity`; Bondesson's two-sided closure theorem (Thm. 7.1.1, p. 106, for the extended class), which was read and is not spent, the node going through the half-line. Feller states his convergence `F_n → F` in the convention of his VIII.1 (convergence on intervals of continuity), recalled on p. 433 of the same section; for a limit that is not defective this is weak convergence of probability laws, and the page of the definition itself was not read.
- **Not admitted (2026-09-19).** The entry grounds a prose-only `[A]` node; no Lean name is declared and none is on the trust boundary. Opened by the author's decision at the review of module B, after the article's numerical run had shown that the families with rational stages stay at a fixed distance from the Poisson member; fidelity row R169.
